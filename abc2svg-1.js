...
var abc2svg={version:"1.15.0-32-g29e7747",vdate:"2017-11-21"}
// abc2svg - abc2svg.js
//
...
/* (the staves are defined) */
// sets {x,y}_note
function draw_basic_note(x, s, m, y_tb, part_name, current_time) {
...
//		return
	...
	if (p) {
		if (!psxygl(x_note, y_note, p)) {
			xygl(x_note, y_note, p, note, part_name, current_time)
		}
	}
...
function draw_note(s,
		   fl, part_name, current_time) {		// fl means draw flags
...
/*fixme:KO when different scales*/
	...
	x = s.x
	for (m = 0; m <= s.nhd; m++)
		draw_basic_note(x, s, m, y_tb, part_name, current_time)
}
...
//--fixme: recall set_scale if different staff
			...
			if (!s.invis) {
				anno_start(s);
				draw_note(s, !bm.s2, p_voice.nm, current_time);
				current_time += s.dur ? s.dur : 0;
				anno_stop(s)
			}
			if (s == bm.s2)
				bm.s2 = null
...
function out_XYAB(str, x, y, a, b, note, part_name, current_time, id) {
	x = sx(x);
	y = sy(y);
	output.push(str.replace(/X|Y|A|B|N|F|T|P|D|G/g, function(c) {
		switch (c) {
...
// output a glyph
function xygl(x, y, gl, note, part_name, current_time) {
// (avoid ps<->js loop)
//	if (psxygl(x, y, gl))
//		return
	var 	tgl = tgls[gl]
